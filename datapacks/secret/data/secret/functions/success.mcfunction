execute as @s[scores={s1fail=0,s1success=0},tag=session1] run tag add @s success
execute as @s[scores={s2fail=0,s2success=0},tag=session2] run tag add @s success
...
execute as @a[scores={timer=60}] run tag add @s remove success
