execute as @s[scores={s1fail=0,s1success=0},tag=session1] run tag @s add success
execute as @s[scores={s2fail=0,s2success=0},tag=session2] run tag @s add success

execute as @a[scores={timer=0}] run scoreboard players set @s timer 1
execute as @a[scores={timer=1..40}] run particle minecraft:enchant ~3 ~5 ~-5 1 1 1 3 3 normal
